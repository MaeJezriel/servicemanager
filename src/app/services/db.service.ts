...
export class DbService {
      ...
      if (res.rows.length > 0) {
        for (var i = 0; i < res.rows.length; i++) { 
          items.push({ 
            ...
            site_address: res.rows.item(i).site_address
           });
        }
      }
  ...
  // Add
  addSong(location_name, tasks_name,time_name, site_equipment, site_address) {
    let data = [location_name, tasks_name, time_name, site_equipment, site_address];
    return this.storage.executeSql('INSERT INTO jobtable (location_name, tasks_name, time_name, site_equipment, site_address) VALUES (?, ?, ?, ?, ?)', data)
    .then(res => {
      this.getSongs();
    });
  }
  ...
  getSong(id): Promise<Job> {
    return this.storage.executeSql('SELECT * FROM jobtable WHERE id = ?', [id]).then(res => { 
      return {
        ...
        site_address: res.rows.item(0).site_address,
      }
    });
  }
  // Update
  updateSong(id, song: Job) {
    let data = [song.site_equipment, song.location_name, song.service_request, song.site_address];
    return this.storage.executeSql(`UPDATE jobtable SET site_equipment = ?, location_name = ?, service_request = ?, site_address = ?, WHERE id = ${id}`, data)
    .then(data => {
      this.getSongs();
    })
  }
